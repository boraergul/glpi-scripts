"""
Gui_templates_export_v2.py
---------------------------
GLPI Template Transfer GUI  —  v2
Yeni işleyiş:
...
import import_templates as imp
...
def do_export(url, app_token, user_token, output_path, qlog):
    """
    Export NotificationTemplates from GLPI to CSV.
    Calls qlog(msg, tag) for live logging.
    Returns the path to the CSV file.
    """
    session = exp.init_session(url, app_token, user_token)
    headers = {"App-Token": app_token, "Session-Token": session,
               "Content-Type": "application/json"}
    qlog("  → Oturum açıldı.", "OK")

    qlog("Notification'lar çekiliyor...")
    all_notifs = exp.fetch_all_pages(url, "Notification", headers)
    notif_id_to_name = {n['id']: n['name'] for n in all_notifs}
    qlog(f"  → {len(all_notifs)} notification.", "OK")

    qlog("Template'ler çekiliyor...")
    all_templates = exp.fetch_all_pages(url, "NotificationTemplate", headers)
    qlog(f"  → {len(all_templates)} template.", "OK")

    qlog("Bağlantılar çekiliyor...")
    all_links = exp.fetch_all_pages(url, "Notification_NotificationTemplate", headers)
    tmpl_to_notifs = {}
    for link in all_links:
        ...
        mode = link.get('mode', '')
        if tid:
            tmpl_to_notifs.setdefault(tid, []).append(
                f"{notif_id_to_name.get(nid, f'ID:{nid}')} [{mode}]")

    fieldnames = ['Template ID', 'Name', 'Item Type', 'Comments', 'CSS',
                  'Date Modified', 'Date Creation', 'Translation Language',
                  'Translation Subject', 'Translation Content HTML',
                  'Translation Content Text', 'Linked Notifications']
    rows = 0
    with open(output_path, 'w', newline='', encoding='utf-8-sig') as f:
        ...
        for tmpl in all_templates:
            tid = tmpl.get('id')
            linked_str = ' | '.join(tmpl_to_notifs.get(tid, []))
            translations = exp.fetch_sub_resource(
                url, "NotificationTemplate", tid,
                "NotificationTemplateTranslation", headers)
            qlog(f"  {tmpl.get('name')} ({len(translations)} çeviri)")
            base = {'Template ID': tid, 'Name': tmpl.get('name'),
                    'Item Type': tmpl.get('itemtype', ''),
                    'Comments': tmpl.get('comment', ''),
                    'CSS': tmpl.get('css', ''),
                    'Date Modified': tmpl.get('date_mod', ''),
                    'Date Creation': tmpl.get('date_creation', ''),
            ...
            if not translations:
                ...
                for tr in translations:
                    writer.writerow({**base,
                                     'Translation Language': tr.get('language', ''),
                                     'Translation Subject': tr.get('subject', ''),
                                     'Translation Content HTML': tr.get('content_html', ''),
                                     'Translation Content Text': tr.get('content_text', '')})
                    rows += 1

    exp.kill_session(url, app_token, session)
    qlog(f"\n✅  Export tamamlandı — {rows} satır", "OK")
    qlog(f"   {os.path.basename(output_path)}", "INFO_C")
    return output_path
...
# ─── Main App ─────────────────────────────────────────────────────────────────
class TemplateTransferApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("GLPI Template Transfer  v2")
        self.configure(bg=BG)
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="📋  GLPI Template Transfer  v2",
    ...
    def _start_export(self, which):
        ...
        if not messagebox.askyesno("Export",
                f"'{key}' ({label}) sunucusundan template'ler export edilsin mi?"):
            return
        self._running = True
    ...
    def _run_export(self, profile, profile_name):
        ...
        try:
            ...
            out = os.path.join(SCRIPT_DIR, f"templates_{safe_name(profile_name)}_{ts}.csv")
    ...
    def _run_transfer(self, src_p, src_name, tgt_p, tgt_name, dry_run):
        ...
        try:
            ...
            ts       = datetime.now().strftime('%Y%m%d_%H%M%S')
            csv_name = f"templates_{safe_name(src_name)}_to_{safe_name(tgt_name)}_{ts}.csv"
            out      = os.path.join(SCRIPT_DIR, csv_name)

            qlog("Oturum açılıyor...")
            csv_path = do_export(
            ...
            qlog("Oturum açılıyor...")
            session = imp.init_session(tgt_p["url"].rstrip('/'),
                                       tgt_p["app_token"], tgt_p["user_token"])
            headers = {"App-Token": tgt_p["app_token"], "Session-Token": session,
                       "Content-Type": "application/json"}
            qlog("  → Oturum açıldı.", "OK")

            qlog("CSV ayrıştırılıyor...")
            templates_data = imp.parse_csv(csv_path)
            qlog(f"  → {len(templates_data)} template", "OK")

            qlog("Mevcut template'ler kontrol ediliyor...")
            existing = imp.fetch_existing_templates(tgt_p["url"].rstrip('/'), headers)
            qlog(f"  → {len(existing)} mevcut\n")

            from collections import defaultdict
            stats = defaultdict(int)
            total = len(templates_data)
            for idx, (name, data) in enumerate(templates_data.items(), 1):
                qlog(f"[{idx}/{total}] {name}")
                tid, action = imp.upsert_template(
                    tgt_p["url"].rstrip('/'), headers, name, data, existing, dry_run)
                stats[action] += 1

                if tid is None:
                    qlog("  ✗ Başarısız", "ERROR")
                    stats['skipped'] += 1
                    continue
                if tid == -1:
                ...
                icon = "✓" if action in ('created', 'updated') else "⚠"
                tag  = "OK" if action in ('created', 'updated') else "WARNING"
                qlog(f"  {icon} {action}  (ID: {tid})", tag)

                existing_trans = imp.fetch_existing_translations(
                    tgt_p["url"].rstrip('/'), headers, tid)
                for lang, tdata in data.get('translations', {}).items():
                    r = imp.upsert_translation(
                        tgt_p["url"].rstrip('/'), headers, tid,
                        lang, tdata, existing_trans, dry_run)
                    stats[r] += 1
                    if 'fail' in r:
                        qlog(f"    ✗ {lang}", "ERROR")
                    elif 'dry' in r:
            ...
            imp.kill_session(tgt_p["url"].rstrip('/'), tgt_p["app_token"], session)

            qlog("\n" + "=" * 50, "HEAD")
            qlog("  ÖZET", "HEAD")
            qlog("=" * 50, "HEAD")
            qlog(f"  Oluşturuldu     : {stats.get('created', 0)}", "OK")
            qlog(f"  Güncellendi     : {stats.get('updated', 0)}", "OK")
            qlog(f"  Çeviri eklendi  : {stats.get('trans_created', 0)}", "OK")
            qlog(f"  Çeviri güncell. : {stats.get('trans_updated', 0)}", "OK")
            fail = (stats.get('create_failed', 0) +
                    stats.get('trans_create_failed', 0) +
                    stats.get('trans_update_failed', 0))
            qlog(f"  Hata            : {fail}", "ERROR" if fail else "INFO")
            if dry_run:
                qlog("\n  ** DRY-RUN: Değişiklik yapılmadı **", "DRY")
            qlog("\n✅  Transfer tamamlandı." if not dry_run else "\n✅  Dry-run tamamlandı.", "OK")
            self.log_queue.put(("__DONE__", "OK"))
